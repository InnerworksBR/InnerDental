...
export function AdminManagement({ canManage }: { canManage: boolean }) {
  ...
  return <section className="management-shell">
    ...
    <div className="management-workspace">
      <nav className="management-tabs" aria-label="Módulos de gestão">{modules.map((item) => <button type="button" className={module === item ? "active" : ""} aria-current={module === item ? "page" : undefined} onClick={() => setModule(item)} key={item}><span>{item.charAt(0)}</span>{item}</button>)}</nav>
      <div className="management-content">
        ...
        {!data ? <div className="management-loading" role="status"><i aria-hidden="true" /><span>Carregando os dados da clínica…</span></div> : <>
          ...
          {module === "Equipe" && <Team team={data.team} canManage={canManage} busy={busy} command={command} />}
...
function scheduleFor(professional: Professional | null | undefined): Record<number, string> {
  const next: Record<number, string> = {};
  for (let day = 0; day <= 6; day += 1) next[day] = professional?.rules.filter((rule) => rule.active && rule.weekday === day).map((rule) => `${timeLabel(rule.start_time)}-${timeLabel(rule.end_time)}`).join(", ") ?? "";
  return next;
}
...
function AgendaManagement({ professionals, canManage, busy, command }: { professionals: Professional[]; canManage: boolean; busy: boolean; command: (payload: Record<string, unknown>, success: string) => Promise<boolean> }) {
  ...
  const [schedule, setSchedule] = useState<Record<number, string>>(() => scheduleFor(selected));
  ...
  async function saveSchedule() {
    const periods = Object.entries(schedule).flatMap(([weekday, value]) => value.split(",").map((item) => item.trim()).filter(Boolean).map((item) => { const [startTime, endTime] = item.split("-").map((part) => part.trim()); return { weekday: Number(weekday), startTime, endTime }; }));
